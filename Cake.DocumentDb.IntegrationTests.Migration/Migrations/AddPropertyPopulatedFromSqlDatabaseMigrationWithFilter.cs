...
{
    [Migration(201707311500)]
    public class AddPropertyPopulatedFromSqlDatabaseMigrationWithFilter : SqlMigration
    {
        public AddPropertyPopulatedFromSqlDatabaseMigrationWithFilter()
        {
            Migrate(m =>
            {
                m.Description("Add property populated from sql with filter");
                ...
                m.Map((log, item, data) =>
                {
                    ...
                    item.Add("sqlPopulatedPropertyWithFilter", new JValue(record.Title));
                });
